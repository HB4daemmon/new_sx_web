...
- 用户已授权：除非当轮另有说明，每轮实际改动完成并通过相应验收后，都应提交并更新远端 `main`。
- 提交前确认改动范围、工作区和远端状态，只提交本轮相关文件；保留用户已有的无关改动。
- 推送前同步远端，优先快进合并；不强推、不覆盖他人提交。验收失败或有未解决冲突时先修复，
  无法安全完成则说明阻塞，不把未验收的改动推到 `main`。
- 这是一轮工作完成后的独立操作，不把 Git 提交或推送加入构建脚本、文档 watcher 或发布脚本。
...
- 游戏验收：`npm test`、`npm run test:content`、`npm run simulate`；
  ...
  构建游戏不发布文档站，构建命令本身不执行 Git 操作；验收完成后的提交与推送遵循上方约定。
...
- Git 提交和远端推送仍是独立操作，按上方“提交与远端同步”约定执行；
  watcher 发布网站本身不触发 Git 操作。
