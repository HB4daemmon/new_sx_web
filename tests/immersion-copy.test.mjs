...
test('UI18.1 keeps primary play surfaces in-world and trims draft meta copy',()=>{
 for(const phrase of [
  '择一，伴此生',
  '入劫将替换当前命途。',
  '此举的牵连',
 ...
 for(const phrase of ['<span class="muted">择一</span>',"['精进','合流','新路']",'draft-lane-head','THE FINAL CHOICE','>详细战报<','>导出<','>导入<'])assert(!app.includes(phrase),phrase);
 assert(app.includes('fighter-side-badge'));
 assert(app.includes("s.current?.type==='boss'?'劫首遗珍'"));
 assert(app.includes("s.current?.type==='elite'?'强敌遗珍'"));
 assert.match(presentation,/UI_REVISION='18\.1'/);
});
