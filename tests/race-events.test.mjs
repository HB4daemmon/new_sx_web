...
test('all ancestry story choices have authored presentation outcomes',()=>{
 const p=c.presentation;assert.equal(p.revision,'17.1');for(const id of ids){const e=c.events.find(x=>x.id===id),copy=p.events[id];assert(copy?.intro);for(const ph of e.phases)for(const ch of ph.choices){const x=copy.choices[`${ph.id}.${ch.id}`];assert(x,`${id}/${ph.id}/${ch.id}`);if(ch.check){assert(x.success&&x.failure);}else assert(x.outcome);}}
});
