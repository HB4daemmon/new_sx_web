...
const raceRates=race.map(x=>x.winRate);
const originRates=origin.map(x=>x.winRate);
const thresholds={raceSpreadMax:15,originSpreadMax:20,originWinRateMin:50,originWinRateMax:80,overallWinRateMin:50,overallWinRateMax:70};
const audit={
  ...
  raceSpread:spread(raceRates),
  originSpread:spread(originRates),
  thresholds,
  race,
  origin,
  matrix,
...
const failures=[];
if(audit.raceSpread>thresholds.raceSpreadMax)failures.push(`race spread ${audit.raceSpread} > ${thresholds.raceSpreadMax}`);
if(audit.originSpread>thresholds.originSpreadMax)failures.push(`origin spread ${audit.originSpread} > ${thresholds.originSpreadMax}`);
if(audit.overallWinRate<thresholds.overallWinRateMin||audit.overallWinRate>thresholds.overallWinRateMax)failures.push(`overall win rate ${audit.overallWinRate} outside ${thresholds.overallWinRateMin}-${thresholds.overallWinRateMax}`);
for(const x of origin)if(x.winRate<thresholds.originWinRateMin||x.winRate>thresholds.originWinRateMax)failures.push(`${x.name} win rate ${x.winRate} outside ${thresholds.originWinRateMin}-${thresholds.originWinRateMax}`);
...
console.log(JSON.stringify({totalRuns:audit.totalRuns,overallWinRate:audit.overallWinRate,raceSpread:audit.raceSpread,originSpread:audit.originSpread,thresholds,race,origin,matrix,topTalents:talents.slice(0,16),failures},null,2));
if(failures.length){console.error(`Balance regression gate failed: ${failures.join('; ')}`);process.exitCode=1;}
