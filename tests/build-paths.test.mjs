...
import {Game,fateOptions,SLOT_ORDER,validateContent,simulateBattle,inRunPool,abilityLoadoutSlot} from '../build/engine.js';
...
function equip(g,ids){g.s.slots=Array(8).fill(null);for(const id of ids){const slot=abilityLoadoutSlot(c.abilities.find(a=>a.id===id));const ix=SLOT_ORDER.findIndex((s,i)=>s===slot&&!g.s.slots[i]);assert(ix>=0);g.s.slots[ix]={id,rank:0};}}
...
test('legacy recipe inspection is descriptive only, sees all metadata and consumes no RNG',()=>{for(const r of c.races){const g=fresh(r.id),before=JSON.stringify(g.s),paths=buildPathProgress(c,g.s);assert.equal(paths.length,22);assert.equal(JSON.stringify(g.s),before);for(const p of paths){assert.equal(p.owned,p.abilities.filter(id=>g.s.slots.some(x=>x?.id===id)).length);assert(p.supports.every(t=>['common',r.id].includes(c.talents.find(x=>x.id===t.id).race)));for(const x of p.pieces)assert.equal(x.available,true);}}});
...
test('each new artifact exercises its real trigger, bounded once per round',()=>{
 const setups={storm:['basic.thunder','aux.qi','art.flag'],ember:['basic.fire','aux.fire'],scale:['basic.fist','aux.stone','strategy.mountain'],sever:['basic.fire','aux.fire','aux.ash'],star:['basic.star','rage.army','aux.focus'],echo:['basic.gale','aux.shadow'],bloom:['basic.spring','aux.breath'],hex:['basic.hex','aux.hex']};
