...
test('rules version advances for build-affecting combat changes',()=>{assert.equal(c.version,'4.1.0');assert.equal(c.rulesVersion,'4.1.0');});
...
test('v5 content and deterministic rules advance together',()=>{assert.equal(c.version,'4.1.0');assert.equal(c.rulesVersion,'4.1.0');});
...
test('v6 content and reward-draft rules advance together',()=>{assert.equal(c.version,'4.1.0');assert.equal(c.rulesVersion,'4.1.0');});
...
test('v8 bridge pieces express both sides of their resource identity',()=>{const tide=c.abilities.find(a=>a.id==='basic.tide'),orbs=c.abilities.find(a=>a.id==='myth.orbs');assert(tide.effects.some(e=>e.type==='gain_shield'));assert(tide.effects.some(e=>e.type==='gain_rage'));const fx=orbs.triggers.flatMap(t=>t.effects);assert(fx.some(e=>e.type==='gain_shield'));assert(fx.some(e=>e.type==='apply_status'&&e.status==='burn'));assert.equal(c.version,'4.1.0');assert.equal(c.rulesVersion,'4.1.0');});
...
test('v11 event rules and content advance together',()=>{assert.equal(c.version,'4.1.0');assert.equal(c.rulesVersion,'4.1.0');assert.equal(c.events.length,56);assert(c.events.every(e=>e.category));});
...
test('v12 character content and deterministic rules advance together',()=>{assert.equal(c.version,'4.1.0');assert.equal(c.rulesVersion,'4.1.0');for(const e of c.events.filter(e=>e.character)){assert(e.episode>=1);assert(e.episodes>=e.episode);assert(e.relationKey);}});
