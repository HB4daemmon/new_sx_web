...
const fighterBadgeClass='fighter-side-badge';
...
export type CompactBattleKind='damage'|'heal'|'shield'|'convert'|'rage'|'miss'|'strategy'|'phase'|'timeout'|'win'|'loss'|'origin'|'ancestry'|'talent'|'stun'|'action';
export interface CompactBattleChange {side:'p'|'e';label:'生命'|'护盾'|'怒气';before:number;after:number;delta:number;}
export interface CompactBattleStatusChange {side:'p'|'e';name:string;status:string;before:number;after:number;delta:number;}
export interface CompactBattlePresentation {
 visible:boolean;
 kind?:CompactBattleKind;
 actor:'p'|'e'|'system';
 actorBadge:'我方'|'敌方'|'天道';
 actorName:string;
 title:string;
 ...
 crit:boolean;
 absorbed:number;
 changes:CompactBattleChange[];
 statuses:CompactBattleStatusChange[];
}
export interface CompactBattleContext {playerName:string;enemyName:string;statusLabels:Record<string,string>;}

const compactKinds=new Set<CompactBattleKind>(['damage','heal','shield','convert','rage','miss','strategy','phase','timeout','win','loss','origin','ancestry','talent','stun','action']);
const compactStateLabels:Record<'hp'|'shield'|'rage','生命'|'护盾'|'怒气'>={hp:'生命',shield:'护盾',rage:'怒气'};
const compactActor=(frame:Frame):'p'|'e'|'system'=>{
 if(frame.actor!=='system')return frame.actor;
 if(frame.sourceId?.startsWith('origin.')||frame.sourceId?.startsWith('strategy.'))return 'p';
 return 'system';
};
const compactFrameTitle=(frame:Frame,amount:number,absorbed:number):{kind?:CompactBattleKind;title:string;outcome?:string}=>{
 switch(frame.kind){
  case 'damage':return {kind:'damage',title:frame.label,outcome:amount>0?`生命损失 ${amount}`:absorbed>0?`护盾吸收 ${absorbed}`:'伤害未造成生命损失'};
  case 'heal':return {kind:'heal',title:frame.label,outcome:`回复 ${amount} 生命`};
  case 'convert':return {kind:'convert',title:frame.label,outcome:amount>0?`转化 ${amount}`:'发生转化'};
  case 'rage':return {kind:'rage',title:frame.label,outcome:'施展怒技'};
  case 'miss':return {kind:'miss',title:frame.label,outcome:'未命中 · 闪避'};
  case 'strategy':return {kind:'strategy',title:frame.label,outcome:'战策判断'};
  case 'phase':return {kind:'phase',title:frame.label,outcome:'阶段转换'};
  case 'timeout':return {kind:'timeout',title:frame.label,outcome:'回合上限'};
  case 'win':return {kind:'win',title:frame.label,outcome:'战斗胜利'};
  case 'loss':return {kind:'loss',title:frame.label,outcome:'战斗失败'};
  case 'origin':return {kind:'origin',title:frame.label,outcome:'入战效果'};
  case 'ancestry':return {kind:'ancestry',title:frame.label,outcome:'族裔触发'};
  case 'talent':return {kind:'talent',title:frame.label,outcome:'天赋触发'};
  case 'stun':return {kind:'stun',title:frame.label,outcome:'跳过行动'};
  case 'action':return {kind:'action',title:frame.label,outcome:'行动'};
  default:return {title:frame.label};
 }
};
const compactFramePresentation=(frame:Frame,previous:Frame|undefined,context:CompactBattleContext):CompactBattlePresentation=>{
 const before=previous??frame,actor=compactActor(frame),actorName=actor==='p'?context.playerName:actor==='e'?context.enemyName:'天道';
 const actorBadge=actor==='p'?'我方':actor==='e'?'敌方':'天道';
 const changes:CompactBattleChange[]=[];
 for(const side of ['p','e'] as const)for(const [key,label] of Object.entries(compactStateLabels) as ['hp'|'shield'|'rage','生命'|'护盾'|'怒气'][]){
  const beforeValue=before[side][key],afterValue=frame[side][key];
  if(beforeValue!==afterValue)changes.push({side,label,before:beforeValue,after:afterValue,delta:afterValue-beforeValue});
 }
 const statuses:CompactBattleStatusChange[]=[];
 for(const side of ['p','e'] as const){
  const name=side==='p'?context.playerName:context.enemyName;
  for(const key of new Set([...Object.keys(before[side].status),...Object.keys(frame[side].status)])){
   const beforeValue=before[side].status[key]??0,afterValue=frame[side].status[key]??0;
   if(beforeValue!==afterValue)statuses.push({side,name,status:context.statusLabels[key]??key,before:beforeValue,after:afterValue,delta:afterValue-beforeValue});
  }
 }
 const amount=Math.max(0,frame.amount??0),absorbed=Math.max(0,frame.breakdown?.absorbed??0),title=compactFrameTitle(frame,amount,absorbed);
 const stateChange=changes.length>0||statuses.length>0;
 const explicit=!!title.kind&&compactKinds.has(title.kind);
 const lowValueSystem=['start','build','ready','wait'].includes(frame.kind);
 const visible=explicit&&!lowValueSystem&&(
  frame.kind!=='action'||stateChange||
  frame.kind==='action'&&!!frame.detail
 )||stateChange&&frame.kind==='status_tick';
 return {visible,kind:title.kind,actor,actorBadge,actorName,title:title.title,outcome:title.outcome,detail:frame.detail,amount,crit:!!frame.crit,absorbed,changes,statuses};
};
export function compactBattleFrame(frame:Frame,previous:Frame|undefined,context:CompactBattleContext):CompactBattlePresentation {
 return compactFramePresentation(frame,previous,context);
}
...
export class FengshenGame extends HTMLElement {
 ...
 async connectedCallback(){
  ...
  try{this.content=window.__GAME_CONTENT__??await fetch('./data/game.json').then(r=>{if(!r.ok)throw new Error('Content request failed');return r.json();});validateContent(this.content);this.selectedOrigin=this.content.origins[0].id;this.selectedPortrait=this.content.origins[0].art;
   ...
   document.addEventListener('keydown',this.keyHandler=e=>{
    ...
    if(e.key==='Tab'&&dialog){
     const items=Array.from(dialog.querySelectorAll<HTMLElement>('button:not([disabled]),summary,input:not([hidden])')).filter(x=>x.getClientRects().length>0),first=items[0],last=items.at(-1);
     if(e.shiftKey&&document.activeElement===first){e.preventDefault();last?.focus();}
     else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first?.focus();}
    }
    if(e.key==='Escape'){
     if(this.modal){e.preventDefault();this.modal='';this.render();}
     else if(this.game?.s.pending){e.preventDefault();this.cancelPendingReplacement();this.save();this.render();}
     return;
    }
    const target=e.target instanceof Element?e.target:null;
    const typing=target?.matches('input,textarea,select,[contenteditable="true"]')||!!target?.closest('button,summary,[role="dialog"]');
    if(e.code==='Space'&&!e.repeat&&!this.home&&!this.modal&&!this.game?.s.talentDraft&&this.game?.s.phase==='battle'&&!typing){
 ...
 cancelPendingReplacement(){
  const g=this.game,pending=g?.s.pending;
  if(!g||!pending)return;
  if(pending.source==='event'){
   this.modal='pending-abandon';
   return;
  }
  g.cancelReplacement();
  this.notify(pending.source==='shop'?'已返回坊市，灵石未扣除。':'已返回奖励选择，机缘仍保留。');
 }
 confirmPendingReplacementAbandon(){
  const g=this.game,pending=g?.s.pending;
  if(!g||!pending||pending.source!=='event')return;
  g.cancelReplacement();
  this.modal='';
  this.notify('已放弃此赠礼；事件结果与命途记录仍保留。');
 }
 ...
 onClick(e:Event){
  ...
  if(rawTarget?.classList.contains('modal-backdrop')&&this.querySelector('.replacement-modal')){
   e.preventDefault();
  ...
  if(this.querySelector('.modal')&&!target.closest('.modal'))return;
  const action=target.dataset.action!,id=target.dataset.id??'',index=Number(target.dataset.index??-1),g=this.game;
  try{
   this.playTone();
   switch(action){
    ...
    case 'skip':if(g!.s.battle){g!.s.battle.cursor=g!.s.battle.frames.length-1;this.paused=false;}break;
    ...
    case 'cancelReplacement':this.cancelPendingReplacement();break;
    case 'keepPendingGift':this.modal='';break;
    case 'confirmPendingAbandon':this.confirmPendingReplacementAbandon();break;
 ...
 rewards(){const s=this.game!.s,title=s.current?.type==='boss'?'劫首遗珍':s.current?.type==='elite'?'强敌遗珍':'机缘';return `<section class="reward-page">${this.heading(title)}<div class="reward-tally"><span>${icon('coin',18)}+${s.rewardInfo?.gold??0}</span><span>${icon('lotus',18)}修为 +${s.rewardInfo?.xp??0}</span></div><div class="reward-cards directed-draft">${s.reward.map((id,i)=>{const eq=s.slots.find(x=>x?.id===id);return `<div class="draft-lane">${this.card(id,eq?Math.min(3,eq.rank+1):0,'reward',`data-index="${i}"`,eq?rankName(eq.rank)+' → '+rankName(Math.min(3,eq.rank+1)):'收下')}${this.fitNote(id)}${btn('inspect','观诀','text-link',`data-id="${id}"`)}</div>`;}).join('')}</div><div class="actions">${btn('skipReward','舍下，继续前行 '+icon('arrow',17),'quiet')}</div></section>`;}
  ...
  fighterFrame(frame:Frame,side:'p'|'e'){
   const f=frame[side],s=this.game!.s,enemy=this.content.enemies.find(e=>e.id===s.battle!.enemyId)!,art=side==='p'?s.portrait:enemy.art,sideLabel=side==='p'?'我方':'敌方';
   const statusRows=Object.entries(f.status).filter(([,v])=>v>0).map(([k,v])=>{
    const label=this.content.labels.statuses[k]??k;
    return `<span class="status ${esc(k)}" role="status" aria-label="${esc(`${f.name}${label}，当前值 ${v}`)}" title="${esc(`${label}当前值 ${v}`)}">${esc(label)} <b>${v}</b></span>`;
   }).join('');
   return `<article class="combatant side-${side} ${side==='e'?'enemy':''} ${frame.actor===side?'acting':''}" aria-label="${esc(`${sideLabel}战斗信息：${f.name}`)}"><div class="fighter-art"><span class="${fighterBadgeClass}">${sideLabel}</span><span class="fighter-aura" aria-hidden="true"></span>${portrait(art,true)}</div><div class="fighter-identity"><h2>${esc(f.name)}</h2>${frame.actor===side?`<span class="turn-mark">${frame.kind==='rage'?'怒技':'出手'}</span>`:''}</div><div class="fighter-bars"><div class="fighter-numbers"><span>生命 <b>${f.hp}</b> / ${f.stats.hp}</span><span class="shield-count" aria-label="${esc(`${f.name}护盾 ${f.shield}`)}">${icon('shield',12)} <span>护盾</span> <b>${f.shield}</b></span></div><div class="bar hp" role="progressbar" aria-label="${esc(`${f.name}生命`)}" aria-valuemin="0" aria-valuemax="${f.stats.hp}" aria-valuenow="${f.hp}"><span style="width:${percent(f.hp,f.stats.hp)}%"></span></div><div class="fighter-numbers"><span>怒气</span><span aria-label="${esc(`${f.name}怒气 ${f.rage} / ${this.content.rules.rageMax}`)}">${f.rage} / ${this.content.rules.rageMax}</span></div><div class="bar rage" role="progressbar" aria-label="${esc(`${f.name}怒气`)}" aria-valuemin="0" aria-valuemax="${this.content.rules.rageMax}" aria-valuenow="${f.rage}"><span style="width:${percent(f.rage,this.content.rules.rageMax)}%"></span></div></div><div class="fighter-core-stats"><span>攻 <b>${f.stats.attack}</b></span><span>防 <b>${f.stats.defense}</b></span><span>速 <b>${f.stats.speed}</b></span></div><div class="combat-statuses" aria-label="${esc(`${f.name}当前状态`)}">${statusRows||'<span class="status status-none" aria-label="无持续状态">无状态</span>'}</div></article>`;
  }
  compactContext(enemyName:string):CompactBattleContext{return {playerName:this.game!.s.name,enemyName,statusLabels:this.content.labels.statuses??{}};}
  statusChanges(frame:Frame,index:number,frames:Frame[]){const enemy=this.content.enemies.find(e=>e.id===this.game!.s.battle!.enemyId)!,view=compactBattleFrame(frame,index>0?frames[index-1]:undefined,this.compactContext(enemy.name));return view.statuses.map(x=>({side:x.side,name:x.name,status:x.status,before:x.before,after:x.after}));}
  ...
  battleLogRow(frame:Frame,index:number,frames:Frame[],enemyName:string,detailed=this.detailedLog){
   const previous=index>0?frames[index-1]:undefined,view=compactBattleFrame(frame,previous,this.compactContext(enemyName));
   if(!view.visible&&!view.statuses.length)return '';
   const actorClass=view.actorBadge==='我方'?'player':view.actorBadge==='敌方'?'enemy':'system';
   const simpleChanges=view.changes.filter(x=>x.delta!==0);
   if(!detailed){
    const primary=view.visible?`<div class="simple-event simple-${view.kind??'event'}"><span class="log-actor-badge ${actorClass}">${view.actorBadge}</span><strong>${esc(view.actorName)}</strong><span>${esc(view.title)}</span>${view.outcome?`<b>${esc(view.crit&&view.kind==='damage'?'暴击 · '+view.outcome:view.outcome)}</b>`:''}${view.absorbed?`<em>护盾吸收 ${view.absorbed}</em>`:''}</div>`:'';
    const changes=simpleChanges.map(change=>{
     const cls=change.label==='生命'?'simple-heal':change.label==='护盾'?'simple-shield':'simple-rage';
     const sign=change.delta>0?'+':'';
     return `<div class="${cls} ${change.side==='p'?'player':'enemy'}"><span>${change.side==='p'?'我方':'敌方'}</span><em>${change.label}</em><b>${change.before} → ${change.after} ${sign}${change.delta}</b></div>`;
    }).join('');
    const statuses=view.statuses.length?`<div class="simple-status-list">${view.statuses.map(x=>`<div class="simple-status ${x.side==='p'?'player':'enemy'}"><span>${x.side==='p'?'我方':'敌方'}</span><strong>${esc(x.name)}</strong><em>${esc(x.status)}</em><b>${x.before} → ${x.after} ${x.delta>0?'+':''}${x.delta}</b></div>`).join('')}</div>`:'';
    const detail=view.detail?`<div class="simple-detail">${esc(view.detail)}</div>`:'';
    return `<article class="battle-log-row simple actor-${actorClass} kind-${view.kind??'event'} ${view.crit?'critical':''}">${primary}${changes}${statuses}${detail}</article>`;
   }
   const details:string[]=[];
   for(const side of ['p','e'] as const){
    const changes=view.changes.filter(x=>x.side===side),statuses=view.statuses.filter(x=>x.side===side),who=side==='p'?this.game!.s.name:enemyName;
    const rows:string[]=[];
    for(const change of changes)rows.push(`<span class="log-delta ${change.label==='生命'?'hp':change.label==='护盾'?'shield':'rage'}">${change.label} ${change.before}→${change.after} <b>${change.delta>0?'+':''}${change.delta}</b></span>`);
    for(const change of statuses)rows.push(`<span class="log-delta status">${esc(change.status)} ${change.before}→${change.after} <b>${change.delta>0?'+':''}${change.delta}</b></span>`);
    if(rows.length)details.push(`<div class="log-side ${side==='p'?'player':'enemy'}"><em>${side==='p'?'我方':'敌方'} · ${esc(who)}</em>${rows.join('')}</div>`);
   }
   const b=frame.breakdown,formula=b?`<div class="combat-formula"><span>原始 ${b.base}</span><span>倍率后 ${b.scaled}</span><span>防御后 ${b.afterDefense}</span><span>易伤后 ${b.afterVulnerable}</span><span>构筑后 ${b.afterBuild}</span><span>暴击后 ${b.afterCrit}</span>${b.absorbed?`<span>护盾吸收 ${b.absorbed}</span>`:''}<b>生命损失 ${b.hpLoss}</b></div>`:'';
   const outcome=view.outcome?`<span class="${view.kind==='damage'?'log-amount':view.kind==='rage'?'log-outcome rage':'log-outcome'}">${esc(view.crit&&view.kind==='damage'?'暴击 · '+view.outcome:view.outcome)}</span>`:'';
   return `<article class="battle-log-row detailed actor-${actorClass} kind-${frame.kind.replace(/[^a-z_-]/g,'')} ${view.crit?'critical':''}"><div class="battle-log-main"><span class="round">${String(frame.round).padStart(2,'0')}</span><span class="log-actor-badge ${actorClass}">${view.actorBadge}</span><span class="log-actor">${esc(view.actorName)}</span><strong>${esc(view.title)}</strong>${outcome}</div>${view.detail?`<div class="log-note">${esc(view.detail)}</div>`:''}${formula}${details.length?`<div class="battle-log-detail">${details.join('')}</div>`:''}</article>`;
  }

 contributionName(id:string){return this.content.abilities.find(a=>a.id===id)?.name??this.content.talents.find(t=>t.id===id)?.name??(id.startsWith('race.')?this.content.races.find(r=>r.id===id.slice(5))?.trait:undefined)??(id.startsWith('fate.')?this.content.fates.find(f=>f.id===this.game!.s.fate)?.name:undefined)??(id==='fate'?this.content.fates.find(f=>f.id===this.game!.s.fate)?.name:undefined)??(id==='status.burn'?'燃烧':id);}
 battleContributionRows(sum:any,d?:Battle['diagnostics']){
  const rows=new Map<string,{damage:number;heal:number;shield:number;rage:number;conversions:number;triggers:number}>();
  const add=(id:string,v:any={})=>{
   const row=rows.get(id)??{damage:0,heal:0,shield:0,rage:0,conversions:0,triggers:0};
   for(const key of ['damage','heal','shield','rage','conversions','triggers'] as const)row[key]+=Number(v?.[key]??0);
   rows.set(id,row);
  };
  for(const [id,v] of Object.entries(sum.contributions??{}))add(id,v);
  for(const source of d?.sources??[])if(source.actor==='p')add(source.id);
  const values=[...rows.entries()].map(([id,v])=>({id,v,activity:v.damage+v.heal+v.shield+v.rage+v.conversions})).sort((a,b)=>b.activity-a.activity||b.v.triggers-a.v.triggers||this.contributionName(a.id).localeCompare(this.contributionName(b.id),'zh-CN'));
  return values.map(({id,v})=>{
   const facts=[v.damage?`实际伤害 ${v.damage}`:'',v.heal?`实际回复 ${v.heal}`:'',v.shield?`提供护盾 ${v.shield}`:'',v.rage?`获得怒气 ${v.rage}`:'',v.conversions?`转化 ${v.conversions} 次`:''].filter(Boolean);
   if(!facts.length)facts.push('本场无直接产出');
   facts.push(`触发 ${v.triggers} 次`);
   return `<div class="contribution-row"><span>${esc(d?this.diagnosticSourceName(d,'p',id):this.contributionName(id))}</span><em>${esc(facts.join(' · '))}</em></div>`;
  }).join('');
 }
 battleContributions(sum:any,d?:Battle['diagnostics']){
  const rows=this.battleContributionRows(sum,d);
  if(!rows)return '';
  return `<div class="battle-contributions"><strong>全部来源</strong>${rows}</div>`;
 }
 ...
 battleSettlement(b:Battle){
  ...
  if(!d)return `<section class="battle-settlement" aria-label="战斗结算"><article><h3>本场关键</h3><p>旧战报记录到的实际来源如下；未记录的来源无法补全。</p>${this.battleContributions(b.summary)||'<p class="muted">暂无可归因的额外贡献。</p>'}</article><article><h3>未生效</h3><p>旧战报未记录触发尝试，无法区分未触发与未出现事件。</p></article><article><h3>主要损耗</h3><p>生命损失 ${b.summary.enemyDamage}；护盾吸收 ${b.summary.playerShieldAbsorbed}；旧战报未记录更细的损耗分类。</p></article></section>`;
  const sourceName=(id:string,actor:'p'|'e'='p')=>this.diagnosticSourceName(d,actor,id);
  const contributionRows=this.battleContributionRows(b.summary,d);
  const keyText=contributionRows?`<div class="battle-contributions"><strong>全部来源</strong>${contributionRows}</div>`:'<p class="muted">本场没有记录到额外来源贡献。</p>';
  ...
  return `<section class="battle-settlement" aria-label="战斗结算"><article><h3>本场关键</h3>${keyText}${actionText}${strategyText}</article><article><h3>未生效</h3>${untriggeredText}</article><article><h3>主要损耗</h3><p>${esc(lossText)}；护盾吸收 ${shieldAbsorbed}；${esc(roundText)}。</p></article>${systemFailure?`<article><h3>系统失败</h3><p>回合上限耗尽，系统结算 ${systemFailure} 点生命；这部分不计入敌方生命损失。</p></article>`:''}${allDetails||actionDetail?`<details class="disclosure" data-panel="battle-diagnostic-detail"><summary>查看全部来源诊断</summary><div class="settlement-details">${actionDetail}${allDetails}</div></details>`:''}</section>`;
 }

  battle(){
   const b=this.game!.s.battle!,cursor=bi(b.cursor,0,b.frames.length-1),f=b.frames[cursor],done=cursor>=b.frames.length-1,enemy=this.content.enemies.find(e=>e.id===b.enemyId)!,sum=b.summary,obj=b.objective,all=b.frames.slice(0,cursor+1).map((row,index)=>({row,index})),context=this.compactContext(enemy.name),visible=all.filter(({row,index})=>{const view=compactBattleFrame(row,index>0?b.frames[index-1]:undefined,context);return view.visible||view.statuses.length>0;}),broken=all.reduce((n,x)=>n+(x.row.actor==='p'?(x.row.breakdown?.absorbed??0):0),0),objective=!done&&obj.type==='survive'?`守至第 ${obj.rounds} 回合`:!done&&obj.type==='break'?`破阵 ${Math.min(broken,obj.value??0)} / ${obj.value}`:!done&&enemy.kind==='boss'?'天劫':'';
   return `${this.heading(done?(b.won?'此战告捷':'此身入劫'):'斗法')}<section class="battle-shell"><div class="battle-context"><span>${done?'历 '+b.rounds+' 回合':f.round?'第 '+f.round+' 回合':'蓄势'}</span>${objective?`<strong>${objective}</strong>`:''}</div><div class="combat-arena">${landscape()}<span class="arena-vignette" aria-hidden="true"></span>${this.fighterFrame(f,'p')}<div class="battle-stage-core"><span class="round-seal">${done?(b.won?'胜':'劫'):f.round||'·'}</span><strong>${esc(done?(b.won?'尘埃落定':'劫数已定'):(f.kind==='ready'?'气机交锋':f.label))}</strong>${!done&&f.detail?`<small>${esc(f.detail)}</small>`:''}</div>${this.fighterFrame(f,'e')}${this.combatVfx(f,cursor,b.frames)}${this.combatFeedback(f,cursor,b.frames)}</div><div class="battle-controls"><div class="speeds" role="group" aria-label="战斗速度">${[1,2,4].map(n=>`<button class="${this.speed===n?'active':''}" data-action="speed" data-index="${n}" aria-pressed="${this.speed===n}" aria-label="${n}倍速">${n}×</button>`).join('')}<button data-action="pause" aria-pressed="${this.paused}" aria-label="${this.paused?'继续播放':'暂停战斗'}" ${done?'disabled':''}>${this.paused?'继续':'暂停'}</button></div>${done?btn('finishBattle',b.won?'收下所得 '+icon('arrow',16):'回望此生','primary small'):btn('skip','跳至结算','small','aria-label="跳过播放，跳至结算"')}</div>${done?this.battleSettlement(b):''}<details class="disclosure battle-data" data-panel="combat-stats"><summary>战局</summary><div class="battle-extra"><div>${['p','e'].map(side=>{const x=f[side as 'p'|'e'];return `<section><strong>${esc(x.name)}</strong><div class="stats-grid">${STAT_KEYS.filter(k=>k!=='hp').map(k=>`<span>${this.content.labels.stats[k]} <b>${x.stats[k]}</b></span>`).join('')}</div></section>`;}).join('')}</div>${done?`<div class="battle-summary"><div><span>造成伤害</span><b>${sum.playerDamage}</b></div><div><span>生命损失</span><b>${sum.enemyDamage}</b></div><div><span>护盾吸收</span><b>${sum.playerShieldAbsorbed}</b></div><div><span>暴击 / 闪避</span><b>${sum.playerCrits} / ${sum.enemyMisses}</b></div><div><span>怒气技</span><b>${sum.playerRageSkills}</b></div><div><span>资源炼化</span><b>${sum.playerConversions}</b></div></div>`:''}${done?this.battleContributions(sum,b.diagnostics):''}</div></details><div class="battle-log-head"><span>战痕</span><button class="text-link log-follow" data-action="followLog" hidden aria-label="回到最新战报">回到最新 ↓</button><span class="log-mode">${this.detailedLog?'详录':'简录'}</span></div><div class="battle-log ${this.detailedLog?'detailed-mode':'simple-mode'}" tabindex="0" role="region" aria-label="可滚动战报">${visible.map(({row,index})=>this.battleLogRow(row,index,b.frames,enemy.name,this.detailedLog)).join('')||'<p class="simple-log-empty">双方蓄势。</p>'}</div></section>`;
  }
 ...
 pendingModal(){
  const p=this.game!.s.pending!,incoming=abilityLoadoutSlot(this.game!.ability(p.id)),sourceText=p.source==='shop'?`来自坊市，确认替换后才扣除 ${p.price} 灵石。`:p.source==='event'?'来自事件赠礼，事件结果已提交；确认替换后才收下赠礼。':'来自战斗奖励，确认替换后才收下。',returnText=p.source==='shop'?'返回坊市，灵石未扣除。':p.source==='event'?'事件结果不可撤回；退出会先确认是否放弃此赠礼。':'返回奖励选择，机缘仍保留。',returnLabel=p.source==='shop'?'返回坊市':p.source==='event'?'放弃此赠礼':'返回奖励';
  return `<div class="modal-backdrop"><section class="modal replacement-modal" role="dialog" aria-modal="true" aria-labelledby="replacement-title" aria-describedby="replacement-description"><button class="icon-button modal-close" data-action="cancelReplacement" aria-label="${esc(returnLabel)}">${icon('close',21)}</button><h2 id="replacement-title">一得，一舍</h2><p id="replacement-description">${slotLabel(incoming)}已满。${sourceText} ${returnText}</p><div class="replacement-incoming">${this.card(p.id,0,'display','','新得')}${this.fitNote(p.id)}</div><div class="replacement-candidates">${this.game!.s.slots.map((eq,i)=>eq&&abilityLoadoutSlot(this.game!.ability(eq.id))===incoming?`<div class="replacement-option">${this.card(eq.id,eq.rank,'replace',`data-index="${i}"`,'舍此')}${this.replacementImpact(p.id,eq.id,i)}</div>`:'').join('')}</div><div class="actions">${btn('cancelReplacement',returnLabel,'quiet',`aria-label="${esc(returnLabel+'，'+returnText)}"`)}</div></section></div>`;
 }
 ...
 else if(this.modal==='pending-abandon')body=`<h2 id="dialog-title">放弃此赠礼？</h2><p>事件结果、已发生的资源变化和命途记录已经提交。确认后只放弃这件尚未收下的赠礼，不会撤销事件结果。</p><div class="actions">${btn('keepPendingGift','保留赠礼','primary','data-autofocus="true"')}${btn('confirmPendingAbandon','放弃此赠礼','quiet')}</div>`;
 ...
 const logFocused=active?.classList.contains('battle-log')??false;
 const wasDialog=!!this.querySelector('.modal'),wasReplacementDialog=!!this.querySelector('.replacement-modal'),oldTalentLevel=this.querySelector<HTMLElement>('[data-talent-level]')?.dataset.talentLevel;
 ...
 const dialog=this.querySelector<HTMLElement>('.modal'),replacementDialog=!!this.querySelector('.replacement-modal');
 const focusSelector=(target:{action?:string;id?:string;index?:string}|undefined)=>{
  if(!target?.action)return '';
  return `[data-action="${CSS.escape(target.action)}"]${target.id!==undefined?`[data-id="${CSS.escape(target.id)}"]`:''}${target.index!==undefined?`[data-index="${CSS.escape(target.index)}"]`:''}`;
 };
 const focusTarget=(selector:string)=>{
  if(!selector)return undefined;
  return Array.from(this.querySelectorAll<HTMLElement>(selector)).find(target=>!target.hasAttribute('disabled')&&target.getClientRects().length>0);
 };
 if(dialog&&(!wasDialog||wasReplacementDialog!==replacementDialog||oldTalentLevel!==this.querySelector<HTMLElement>('[data-talent-level]')?.dataset.talentLevel)){if(focus?.action&&(!wasDialog||!replacementDialog))this.focusReturn=focusSelector(focus);(dialog.querySelector<HTMLElement>('[data-autofocus="true"]')??dialog.querySelector<HTMLElement>('button:not([disabled]),summary'))?.focus({preventScroll:true});}
 else if(!dialog&&wasDialog&&this.focusReturn){
  const selector=this.focusReturn;this.focusReturn='';focusTarget(selector)?.focus({preventScroll:true});
 }
 else if(focus?.action&&same)focusTarget(focusSelector(focus))?.focus({preventScroll:true});
 ...
 if(logFocused&&same&&!dialog)this.querySelector<HTMLElement>('.battle-log')?.focus({preventScroll:true});
