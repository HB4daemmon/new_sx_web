import {Game,Content,RunState,LoadoutSlot,Stat,Equipped,Frame,Battle,SLOT_ORDER,STAT_KEYS,MECHANIC_TAG_LABELS,fateOptions,conditionOK,value,validateContent,inRunPool,routeLength,abilityLoadoutSlot,abilityMechanicTags,slotLabel,storyOpportunitiesForRelation} from './engine.js';
...
import {UI_REVISION,rankName,threadText,conditionText,narrativeAvailable,choiceCopy,storyText,resultText,abilityLines,abilityBehaviorLines,abilityBehaviorSummary,choiceEffects,phaseText,creationSummary,creationHint,creationLoadout} from './presentation.js';
declare global {interface Window{__GAME_CONTENT__?:Content;}}
const SAVE_KEY='fengshen-run-v43';
const PREVIOUS_SAVE_KEY='fengshen-run-v42';
...
export class FengshenGame extends HTMLElement {
 ...
 async connectedCallback(){
  ...
  try{this.content=window.__GAME_CONTENT__??await fetch('./data/game.json').then(r=>{if(!r.ok)throw new Error('Content request failed');return r.json();});validateContent(this.content);this.selectedOrigin=this.content.origins[0].id;this.selectedPortrait=this.content.origins[0].art;
   try{this.legacySave=localStorage.getItem(PREVIOUS_SAVE_KEY)??localStorage.getItem(LEGACY_SAVE_KEY)??localStorage.getItem(LEGACY_OLD_SAVE_KEY)??'';const text=localStorage.getItem(SAVE_KEY);if(text)this.game=Game.load(this.content,text);this.muted=localStorage.getItem('fengshen-muted')!=='false';this.detailedLog=localStorage.getItem('fengshen-detailed-log')==='true';}catch{this.toastText='存档不可读取，可重新入劫或导入存档。';}
 ...
 characterClue(character:CharacterEntry):string{
  const s=this.game!.s,possible=storyOpportunitiesForRelation(this.content,s,character.key).length>0;
  if(s.phase==='map')return possible?`本幕前方的普通奇遇，可能传来${character.name}的消息；路线仍需自行选择。`:`本幕暂未发现${character.name}的明确机会，追踪会等候后续未解析的奇遇。`;
 ...
 else if(this.modal==='settings')body=`<h2 id="dialog-title">静室</h2><div class="setting-row"><span>声息</span>${btn('sound',this.muted?'关':'开','small',`role="switch" aria-label="音效" aria-checked="${!this.muted}"`)}</div><div class="setting-row"><span>战痕详录</span>${btn('detailedLog',this.detailedLog?'开':'关','small',`role="switch" aria-label="详细战报" aria-checked="${this.detailedLog}"`)}</div><div class="setting-row"><span>命书${this.storageOK?' · 已封存':' · 封存有误'}</span><div class="row">${btn('export','留存','small',this.game?'':'disabled')}${btn('import','载入','small')}</div></div>${this.legacySave?`<div class="setting-row"><span>旧卷 · 4.2及更早</span>${btn('exportLegacy','留存旧卷','small')}</div>`:''}<div class="setting-row"><span>修行札</span>${btn('help','翻阅','small')}</div>${this.game?`<div class="setting-row"><span>另起命途</span>${btn('new','再入此劫','small')}</div>`:''}<details class="disclosure" data-panel="version"><summary>卷尾</summary>${this.game?`<p>命数 ${esc(this.game.s.seed)}</p>`:''}<p>卷本 ${esc(this.content.version)} · 法则 ${esc(this.content.rulesVersion)}</p><p>书页 ${UI_REVISION}</p></details>`;
 ...
 else if(this.modal==='ability'){const a=this.content.abilities.find(x=>x.id===this.modalAbility)!,eq=this.game?.s.slots.find(x=>x?.id===a.id),rank=eq?.rank??0,mechanics=abilityMechanicTags(a);body=`<h2 id="dialog-title">功法详录</h2>${this.card(a.id,rank,'display')}<div class="ability-tags">${mechanics.map(x=>`<span>${esc(TAGS[x])}</span>`).join('')||'<span>无机制 Tag</span>'}</div>${a.relicConversions?.length?`<div class="build-rule"><strong>命器转换</strong>${a.relicConversions.map(x=>`<p>${esc(x.label??a.name)}：${esc(x.from)} → ${esc(x.to)} · ${value(x.percent,rank)}%</p>`).join('')}</div>`:''}<details class="disclosure" data-panel="rank-effects"><summary>各阶变化</summary><div class="rank-effects">${[0,1,2,3].map(n=>`<section class="${n===rank?'current':''}"><strong>${rankName(n)}</strong>${abilityLines(this.content,a,n).map(x=>`<p>${esc(x)}</p>`).join('')}</section>`).join('')}</div></details>`;}
